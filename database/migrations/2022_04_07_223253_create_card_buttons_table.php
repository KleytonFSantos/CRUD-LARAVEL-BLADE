...
class CreateCardButtonsTable extends Migration
{
    ...
    public function up()
    {
        if(!Schema::hasTable('card_buttons')){
            Schema::create('card_buttons', function (Blueprint $table) {
                $table->id();
                $table->unsignedBigInteger('card_id');
                $table->foreign('card_id')->references('id')->references('cards');
                $table->unsignedBigInteger('button_type_id');
                $table->foreign('button_type_id')->references('id')->references('button_types');
                $table->enum('button_size', ['small', 'large','extra_large']);
                $table->integer('position');
                $table->string('button_text');
                $table->string('button_value');
                $table->string('button_icon_value');
